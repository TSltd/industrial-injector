...
A double four bar, 5 point linkage made up of points A, B, C, D and E such that:



Point A has a y value to be optimized or determined by user input (separation of points at platten) and a dynamic x value (representing the input motion)

Point A is connected to Point B by line AB of length L1

Point C has a fixed y value determined by user input and a fixed x value such as 0

Point B is connected to Point C by line BC of length L2

Point B is connected to Point D by line BD of length T1 

Point B is connected to Point C by line BC of length T2

Point D is connected to Point E by line DE of length L3

Point E is connected to -E by line L4 (crosshead)
...
Points A, B, C, D and E are mirrored in the y axis (negative y values plotted simultaneously such that y=0 represents the centreline

## Input 

- Desired clamping force 
- Maximum input Force 
- Desired minimum mould opening stroke
